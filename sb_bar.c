...
static void DrINumber(signed int val, int x, int y)
{
    ...
    if (val < 0)
    {
        if (val < -9)
        {
            V_DrawPatch(x + 1, y + 1, 0, W_CacheLumpName(DEH_String("LAME"), PU_CACHE));
        }
        else
        {
            val = -val;
            V_DrawPatch(x + 18, y, 0, PatchINumbers[val]);
            V_DrawPatch(x + 9, y, 0, PatchNEGATIVE);
        }
        return;
    }
    if (val > 99)
...
void SB_Drawer(void)
{
    ...
    else
    {
        if (SB_state == -1)
        {
            V_DrawPatch(0, 158, 0, PatchBARBACK);
            if (players[consoleplayer].cheats & CF_GODMODE)
            {
                V_DrawPatch(16, 167, 0, 
                            W_CacheLumpName(DEH_String("GOD1"), PU_CACHE));
                V_DrawPatch(287, 167, 0, 
        ...
        if (!inventory)
        {
            ...
            {
                // Main interface
                V_DrawPatch(34, 160, 0, PatchSTATBAR);
                oldarti = 0;
                oldammo = -1;
                oldarmor = -1;
                oldweapon = -1;
        ...
        else
        {
            if (SB_state != 1)
            {
                V_DrawPatch(34, 160, 0, PatchINVBAR);
            }
            DrawInventoryBar();
            SB_state = 1;
        }
    ...
    if (CPlayer->powers[pw_flight])
    {
            ...
            || !(CPlayer->powers[pw_flight] & 16))
        {
            ...
            if (CPlayer->mo->flags2 & MF2_FLY)
            {
                ...
                {
                    V_DrawPatch(20, 17, 0, W_CacheLumpNum(spinflylump + 15,
                                                       PU_CACHE));
                }
                else
                {
                    V_DrawPatch(20, 17, 0, W_CacheLumpNum(spinflylump + frame,
                                                       PU_CACHE));
...
void DrawCommonBar(void)
{
    ...
    V_DrawPatch(0, 148, 0, PatchLTFCTOP);
    V_DrawPatch(290, 148, 0, PatchRTFCTOP);
    ...
    if (oldhealth != HealthMarker)
    {
        ...
        V_DrawPatch(0, 190, 0, PatchCHAINBACK);
        V_DrawPatch(2 + (healthPos % 17), chainY, 0, PatchCHAIN);
        V_DrawPatch(17 + healthPos, chainY, 0, PatchLIFEGEM);
        V_DrawPatch(0, 190, 0, PatchLTFACE);
        V_DrawPatch(276, 190, 0, PatchRTFACE);
        ShadeChain();
...
//    int i;
    ...
    if (ArtifactFlash)
    {
        V_DrawPatch(180, 161, 0, PatchBLACKSQ);

        temp = W_GetNumForName(DEH_String("useartia")) + ArtifactFlash - 1;

        V_DrawPatch(182, 161, 0, W_CacheLumpNum(temp, PU_CACHE));
        ArtifactFlash--;
...
/*
    if (deathmatch)
    {
        ...
        if (temp != oldfrags)
        {
            V_DrawPatch(57, 171, 0, PatchARMCLEAR);
...
*/
    ...
    if (oldkeys != playerkeys)
    {
        ...
        {
            V_DrawPatch(153, 164, 0, W_CacheLumpName(DEH_String("ykeyicon"), PU_CACHE));
        }
        if (CPlayer->keys[key_green])
        {
            V_DrawPatch(153, 172, 0, W_CacheLumpName(DEH_String("gkeyicon"), PU_CACHE));
        }
        if (CPlayer->keys[key_blue])
        {
            V_DrawPatch(153, 180, 0, W_CacheLumpName(DEH_String("bkeyicon"), PU_CACHE));
        }
        oldkeys = playerkeys;
        UpdateState |= I_STATBAR;
    }
    ...
    {
        V_DrawPatch(108, 161, 0, PatchBLACKSQ);
        if (temp && CPlayer->readyweapon > 0 && CPlayer->readyweapon < 7)
        {
            DrINumber(temp, 109, 162);
            V_DrawPatch(111, 172, 0, 
    ...
    {
        V_DrawPatch(224, 171, 0, PatchARMCLEAR);
...
void DrawInventoryBar(void)
{
    ...
    for (i = 0; i < 7; i++)
    {
            ...
            && CPlayer->inventory[x + i].type != arti_none)
        {
            ...
            V_DrawPatch(50 + i * 31, 160, 0, W_CacheLumpName(patch, PU_CACHE));
            DrSmallNumber(CPlayer->inventory[x + i].count, 69 + i * 31, 182);
        }
    }
    V_DrawPatch(50 + curpos * 31, 189, 0, PatchSELECTBOX);
/*
    if (x != 0)
    {
        V_DrawPatch(38, 159, 0, !(leveltime & 4) ? PatchINVLFGEM1 :
                    PatchINVLFGEM2);
    }
*/
    if (CPlayer->inventorySlotNum - x > 7)
    {
        V_DrawPatch(269, 159, 0, !(leveltime & 4) ?
...
*/
    if (!inventory)
    {
        if (CPlayer->readyArtifact > 0)
        {
            ...
            V_DrawPatch(286, 170, 0, W_CacheLumpName(patch, PU_CACHE));
    ...
    else
    {
        ...
        for (i = 0; i < 7; i++)
        {
            ...
            {
                patch = DEH_String(patcharti[CPlayer->inventory[x + i].type]);
                V_DrawPatch(50 + i * 31, 168, 0, 
                            W_CacheLumpName(patch, PU_CACHE));
                DrSmallNumber(CPlayer->inventory[x + i].count, 69 + i * 31,
                              190);
            }
        }
        V_DrawPatch(50 + curpos * 31, 197, 0, PatchSELECTBOX);
/*
        if (x != 0)
        {
            V_DrawPatch(38, 167, 0, !(leveltime & 4) ? PatchINVLFGEM1 :
                        PatchINVLFGEM2);
        }
*/
        if (CPlayer->inventorySlotNum - x > 7)
        {
            V_DrawPatch(269, 167, 0, !(leveltime & 4) ?
                        PatchINVRTGEM1 : PatchINVRTGEM2);
        }
    }
}
