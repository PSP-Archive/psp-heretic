...
void F_DemonScroll(void)
{
    ...
    if (finalecount < 70)
    {
        memcpy(screens[0], p1, SCREENHEIGHT * SCREENWIDTH);
        nextscroll = finalecount;
        return;
    }
    if (yval < 64000)
    {
        memcpy(screens[0], p2 + SCREENHEIGHT * SCREENWIDTH - yval, yval);
        memcpy(screens[0] + yval, p1, SCREENHEIGHT * SCREENWIDTH - yval);
        yval += SCREENWIDTH;
        nextscroll = finalecount + 3;
    }
    else
    {                           //else, we'll just sit here and wait, for now
        memcpy(screens[0], p2, SCREENWIDTH * SCREENHEIGHT);
    }
}
...
//                I_ResetScaleTables(palette);
		...
		V_DrawPatch(0, 0, 0, W_CacheLumpName(DEH_String("TITLE"), PU_CACHE));
	    else
...
void F_Drawer(void)
{
    ...
    else
    {
        switch (gameepisode)
        {
			...
			V_DrawPatch(0, 0, 0, W_CacheLumpName(DEH_String("ORDER"), PU_CACHE));
                }
                else
                {
