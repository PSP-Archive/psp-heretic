...
*/
        ...
        case 1:                // leaving old level
            if (gameepisode < 4)
            {
                V_DrawPatch(0, 0, 0, patchINTERPIC);
                IN_DrawOldLevel();
            }
            break;
    ...
    if (prevmap == 9)
    {
        for (i = 0; i < gamemap - 1; i++)
        {
            V_DrawPatch(YAHspot[gameepisode - 1][i].x,
                        YAHspot[gameepisode - 1][i].y, 0, patchBEENTHERE);
        }
        if (!(intertime & 16))
        {
            V_DrawPatch(YAHspot[gameepisode - 1][8].x,
                        YAHspot[gameepisode - 1][8].y, 0, patchBEENTHERE);
        }
    }
    else
    {
        ...
        if (!(intertime & 16))
        {
            V_DrawPatch(YAHspot[gameepisode - 1][prevmap - 1].x,
                        YAHspot[gameepisode - 1][prevmap - 1].y, 0, 
                        patchBEENTHERE);
        }
    }
}
...
void IN_DrawYAH(void)
{
    ...
    {                           // draw the destination 'X'
        V_DrawPatch(YAHspot[gameepisode - 1][gamemap - 1].x,
                    YAHspot[gameepisode - 1][gamemap - 1].y, 0, patchGOINGTHERE);
    }
}
